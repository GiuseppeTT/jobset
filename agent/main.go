...
var (
	...
	workerTerminationPeriod   = flag.Duration("worker-termination-grace-period", 0, "Termination grace period of the worker when it must be killed")
	criSocketPath             = flag.String("cri-socket-path", "", "Path of the CRI socket")
	criPollingInterval        = flag.Duration("cri-polling-interval", 0, "Polling interval for the CRI API")
