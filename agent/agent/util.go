...
func (a *Agent) setCountVariablesToRedis(ctx context.Context, restartCount *int, totalRestartCount *int) error {
	if restartCount == nil || totalRestartCount == nil {
		return fmt.Errorf("restart count and total restart count cannot be nil")
	}
	_, err := a.redisClient.TxPipelined(ctx, func(transaction redis.Pipeliner) error {
		transaction.HSet(ctx, a.buildRedisRestartCountKeyName(), a.workerId, restartCount)
		transaction.HSet(ctx, a.buildRedisTotalRestartCountKeyName(), a.workerId, totalRestartCount)
		return nil
	})
	if err != nil {
		return fmt.Errorf("failed to set restart count and total restart count in transaction to Redis: %w", err)
	}
	return nil
}
