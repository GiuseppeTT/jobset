...
func (a *Agent) handleWorkerRestartedCase(ctx context.Context) {
	...
	err := a.setCountVariablesToRedis(ctx, newRestartCount, newTotalRestartCount)
	if err != nil {
		klog.Errorf("Failed to set restart count and total restart count in transaction to Redis: %v", err)
		return
	}
